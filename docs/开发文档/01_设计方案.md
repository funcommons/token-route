...
| V2.2 | 2026-09-04 | justin | **亲和管理端点**（issue #1）：对既有亲和键的内部运维写面 `set/get/list/delete`（`/v1/admin/affinity/**`，TR-ADM-001~004）——「零管理写面」原则**唯一例外**（内部鉴权 `X-Internal-Service-Token` + trusted-ips + 审计日志 `[TR-AFFINITY-ADMIN]`）；不动 resolve 流程 / 不动 FEED 数据面 / 不新增键族，upsert 即改流（下一 resolve 亲和命中新指向，冻结让路语义不变）。动因：MMagiX 平台 pin 功能下线（14 号方案 V2.1），运营手动切流改由改亲和值承接；原「pin 一等公民」需求撤销（issue 编辑历史存档） |
...
| **亲和管理面**（V2.2，唯一写面例外） | **内部鉴权**（不走三模式）：`X-Internal-Service-Token` 恒时比对 `tr.auth.affinity-admin.internal-token`（空 = 端点整体禁用）+ `trusted-ips` 白名单（空 = 不校验）；与 MMagiX 内部服务鉴权同模式 | `/v1/admin/affinity/**` |

> 原管理写面（`/v1/admin/**` CRUD）V2.0 整体删除；`/v1/refresh/**` 跟随契约面鉴权模式。V2.2 例外恢复 `/v1/admin/affinity/**` 子前缀（仅亲和 runtime 键，issue #1），拦截器与三模式互斥注册。
