...
| V2.2 | 2026-09-04 | justin | **内部管理端点**（issue #1）：①**亲和管理**——对既有亲和键的运维写面 `set/get/list/delete`（`/v1/admin/affinity/**`，TR-ADM-001~004），upsert 即改流（下一 resolve 亲和命中新指向，冻结让路语义不变）；②**状态重置**——`/v1/admin/state/reset`（TR-ADM-005），条目从 FROZEN/降级态即时复位 ACTIVE（余额不足停用 → 充值后手动恢复），原子清 frozen_until/freeze_count/连败/1m 窗。两者合称「零管理写面」原则**唯一例外**（内部鉴权 `X-Internal-Service-Token` + trusted-ips + 审计日志）；不动 resolve 流程 / 不动 FEED 数据面 / 亲和复用既有键族。动因：MMagiX 平台 pin 功能下线（14 号方案 V2.1），运营手动切流/手动恢复改由内部管理面承接；原「pin 一等公民」需求撤销（issue 编辑历史存档） |
| V2.3 | 2026-09-04 | justin | **嵌入式核心 SDK（库化）**：核心（表种子/脚本引擎/resolve/report/ops/admin/评估器）自 token-route-app 拆出为 **`token-route-starter`** 模块（包名不变，git mv 保历史），Spring Boot 3 自动装配（`TrStarterAutoConfiguration`，总开关 `tr.enabled`，缺省开）——消费方 Java 服务进程内直调 `TrRouteEngine`（resolve/report/detach 门面），**零网络跳**；状态仍全在 Redis，与独立服务可混布同键空间（§7.2）。评估器由 @Scheduled 改自管 daemon 线程（`TrEvaluatorScheduler`，不动宿主调度语义）。app 瘦身为 HTTP 壳。**补齐 TR-CTR-003**：detach 内核（TrRouteEngine.detach + app `POST /v1/affinity/detach`）——契约/OpenAPI/SDK 已有而实现缺失的缺口。接入指南 `docs/用户文档/03_嵌入式SDK接入指南.md`。影响注记：嵌入式消「+1 跳」后，§11#2 高 QPS 准入顾虑对嵌入式形态不再成立（升级耦合换部署简洁，指南 §0 对比表） |
| V2.4 | 2026-09-05 | justin | **TokenGo 接入确认（issue #2~#5，R1~R4）**：① FEED 三态刷新用例落仓（`TrFeedThreeTriggerIT`：冷启动回源/懒刷新保旧值后台收敛/强制 ping 立即生效，mock 上游可热改）；② 表定义增 `metadata` 版本追溯字段（可选、不进内核语义，策略系统发布种子写版本号，TR-OPS-001 原样回显）+ 种子「发布→生效→回滚」用例（`TrSeedPublishRollbackIT`：换种子 registry 即换版本视图，Redis/FEED 不动）；③ 任务面 fallback 用例（gateway-demo 第 ⑥ 步：DISABLE_FAIL 立即摘除 → 确定性换上游，实测通过）；④ TokenGo 门户只读运维对接走查文档（`docs/用户文档/TokenGo只读运维页对接走查.md`：接口清单/鉴权/10 分钟走查/护栏——本仓近零改动确认） |
...
| **亲和管理**（V2.2，issue #1） | 对**既有亲和键**的内部运维写面：set（upsert 指向 + ttl）/ get（指向 + 剩余 TTL）/ list（session 前缀 SCAN）/ delete（幂等解除）——运营手动切流由改亲和值承接；**「零管理写面」的唯一例外**，内部鉴权（§6.1）+ 审计日志 + 亲和事件 ring（by=ADMIN_*） |
| **状态重置**（V2.2，issue #1 追加） | `state/reset`：条目从状态机判定态（DEGRADED_*/FROZEN）即时复位 ACTIVE（余额不足停用 → 充值后手动恢复，不等冻结退避窗）；原子清 frozen_until / freeze_count（退避从头计）/ 连败计数 / 1m 窗；OFFLINE 拒绝（FEED 管理位）；report 三态照常驱动（无豁免）——与亲和管理共用内部鉴权组 |
...
- ❌ 无管理写面：不提供任何 admin CRUD API（表壳 config 种子化）；不引入注册中心（对齐 12 号 ADR-0002）。**唯一例外**：亲和管理端点组 `/v1/admin/affinity/**`（V2.2，§6.2 TR-ADM）——仅作用于亲和 runtime 键，不触表壳/条目数据。
...
| **内部管理面**（V2.2，唯一写面例外） | **内部鉴权**（不走三模式）：`X-Internal-Service-Token` 恒时比对 `tr.auth.affinity-admin.internal-token`（空 = 端点整体禁用）+ `trusted-ips` 白名单（空 = 不校验）；与 MMagiX 内部服务鉴权同模式 | `/v1/admin/**`（亲和管理 + 状态重置） |

> 原管理写面（`/v1/admin/**` CRUD）V2.0 整体删除；`/v1/refresh/**` 跟随契约面鉴权模式。V2.2 例外恢复 `/v1/admin/**` 子前缀（仅亲和 runtime 键 + 条目状态复位，issue #1），拦截器与三模式互斥注册。
...
| 7 | TR-OPS-003 | GET /v1/ops/affinity-events?table_id&type | 亲和事件史（BIND/DETACH，含 ADMIN_* 来源） |
| 8 | TR-OPS-004 | GET /v1/ops/state-logs?entry_id | 状态迁移史（from/to/原因/时刻） |

**亲和管理面（V2.2 唯一写面例外，issue #1；内部鉴权 §6.1）**

| # | 编号 | 接口 | 用途 |
|---|---|---|---|
| 9 | TR-ADM-001 | POST /v1/admin/affinity/set {table_id, session_id, entry_id, ttl_seconds?} | upsert 亲和指向（entry 存在性校验 10400；ttl 缺省沿用表级空闲超时，1~604800s）；**改流 = 下一 resolve 亲和命中新指向** |
| 10 | TR-ADM-002 | GET /v1/admin/affinity/get?table_id&session_id | 查当前指向 + 剩余 TTL（无绑定 found=false） |
| 11 | TR-ADM-003 | GET /v1/admin/affinity/list?table_id&session_id_prefix&limit | 前缀 SCAN 排查（≤500 条；超限 complete=false 收窄前缀） |
| 12 | TR-ADM-004 | POST /v1/admin/affinity/delete {table_id, session_id} | 解除亲和（幂等，无绑定 deleted=false） |
| 13 | TR-ADM-005 | POST /v1/admin/state/reset {table_id, entry_id} | 条目状态即时复位 ACTIVE（FROZEN/DEGRADED_*；原子清 frozen_until/freeze_count/连败/1m 窗 + 迁移史 ADMIN_RESET；OFFLINE 拒绝 10100；已 ACTIVE 无残留幂等 reset=false） |

> 亲和管理语义：不动 resolve 流程（亲和判定位置 / 冻结让路自愈不变——指向 FROZEN 条目 → 亲和 miss → 正常选择）；指向条目被状态机冻结时消费方失败信号照常驱动；全操作审计日志 `[TR-AFFINITY-ADMIN]` + 亲和事件 ring（by=ADMIN_SET/ADMIN_DELETE，TR-OPS-003 可查）。
...
| **嵌入式（V2.3）** | **`token-route-starter`**：核心（表种子/脚本引擎/resolve/report/ops/admin/评估器）为可嵌入库，Spring Boot 3 自动装配（`tr.enabled` 总开关）——宿主 Java 服务进程内直调 `TrRouteEngine` 门面，**零网络跳**；状态全在 Redis，与独立服务可混布同键空间；表种子随宿主配置。接入指南 `docs/用户文档/03_嵌入式SDK接入指南.md` |
